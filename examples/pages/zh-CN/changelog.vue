    ...
    mounted() {
      ...
      for (let len = changeLogNodes.length, i = 2; i < len; i++) {
        ...
        if (node.tagName !== 'H3') {
          ...
          fragments += `</li><li><h3><a href="https://github.com/GavinHome/Albatro-UI/releases/tag/v${release}" target="_blank">${release}</a></h3>`;
        }
      }
      fragments = fragments.replace(/#(\d+)/g, '<a href="https://github.com/GavinHome/Albatro-UI/issues/$1" target="_blank">#$1</a>');
