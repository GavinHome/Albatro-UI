...
export default {
  ...
  mounted() {
    ...
    this.$event.$on(ACTION_APPLY_THEME, val => {
      this.userConfig = val;
      this.onAction();
    });
    this.$event.$on(ACTION_DOWNLOAD_THEME, (themeConfig, themeName) => {
  ...
  methods: {
    ...
    updateDocs(cb) {
      ...
      this.$event.$emit(ACTION_USER_CONFIG_UPDATE, this.userConfig);
