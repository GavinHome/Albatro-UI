...
export default {
  ...
  methods: {
    ...
    onDownload() {
      this.$event.$emit(ACTION_DOWNLOAD_THEME, this.userConfig, this.previewConfig.name);
    },
    onAction() {
      this.onUserConfigUpdate(this.userConfig);
      this.$event.$emit(ACTION_APPLY_THEME, this.userConfig);
    },
    undo() {
    ...
    onSelectChange(val) {
      this.$event.$emit(ACTION_COMPONECT_SELECT, val);
